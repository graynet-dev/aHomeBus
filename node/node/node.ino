...
void setup() {
  ...
  //Serial.print(F("Attach Node logic..."));if(!ahb0.nodeAttach(&ahbNode0)) {Serial.println(F("Error!"));}else{Serial.println(F("Done!"));}
...
void CAN_test (){
  if (Serial.available()>0){
   ...
   // else if (inByte == 3) {ahbCan0.mcp2515_initCANBuffers(); Serial.println("Pres 3");}
    else if (inByte == 4) { Serial.print(ahbCan0.checkError()); Serial.println(" checkError Pres 4");}
    else if (inByte == 5) { Serial.print(ahbCan0.errorCountRX()); Serial.println(" errorCountRX Pres 5");}
    else if (inByte == 6) { Serial.print(ahbCan0.errorCountTX()); Serial.println(" errorCountTX Pres 6");}
    else if (inByte == 7) { Serial.print(ahbCan0.getError()); Serial.println(" getError Pres 7");}
    //mcp2515_init(const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock)
    //getError(void)

    else if (inByte == 8) while(1);
    else if (inByte == 9) {
           byte data1[2] = {50,1};
