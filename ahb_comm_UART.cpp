...
AHB_UART::AHB_UART(Stream &serial, uint8_t *route_table , uint8_t netsize) {
        
        AHB_UART::_interface = &serial;
        AHB_UART::_buf[0] = 0;
...
uint8_t AHB_UART::address_network_get(uint8_t i){
     return all_network_address[i];   
}
 


uint8_t AHB_UART::net_size_get(void){
return net_size;
}

uint8_t AHB_UART::busType(){
        return type_UART;
}

//bool AHB_UART::ahbSend_V(byte type, unsigned int target, unsigned int source, char port, byte len, const byte *data) {
...
bool AHB_UART::bufShift(byte len) {
...
bool AHB_UART::ahbReceive_V(ahbPacket &pkg) {
        byte read;
        byte state;
        byte curwrite;
        bool retry = false;
                ...
                if(_buf[0] == 0) {  //No active RX, ignore everything until SOH
                    ...
                    if(_buf[(_buf[0]-1)] == 0x04 && _buf[0] >= 10) {
                        ...
                        for(read = 2; read <= _buf[0] && !break2; read++) {
                            switch(state) {
                                case 0:
                                    pkg.meta.type = ahbHexToByte(_buf[read]);
                                    state++;
                                    break;
                                case 1:
                                    if(_buf[read] != 0x1F) {
                                ...
                                case 2:
                                    if(_buf[read] == 0x1F) {
                                        ...
                                        pkg.meta.target <<=4;
                                        pkg.meta.target |= ahbHexToByte(_buf[read]);
                                    }
                                    break;
                                case 3:
                                    if(_buf[read] == 0x1F) {
                                        ...
                                        pkg.meta.source <<=4;
                                        pkg.meta.source |= ahbHexToByte(_buf[read]);
                                    }
                                    break;
                                case 4:
                                    if(_buf[read] == 0x1F) {
                                        ...
                                        pkg.meta.port <<=4;
                                        pkg.meta.port |= ahbHexToByte(_buf[read]);
                                    }
                                    break;
                                case 5:
                                    pkg.len = ahbHexToByte(_buf[read]);
                                    state++;
                                    break;
                                case 6:
                                    if(_buf[read] != 0x02) {
                                ...
                                case 7:
                                    if(_buf[read] == 0x1F) {
                                        ...
                                        pkg.data[curwrite] <<=4;
                                        pkg.data[curwrite] |= ahbHexToByte(_buf[read]);
                                    }
                                    break;
                                case 8:
                                    bufShift(read);
...
byte AHB_UART::ahbHexToByte(byte hex) {
